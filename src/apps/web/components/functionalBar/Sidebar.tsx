...
const MENU_ITEMS = [
  ...
  { id: 'teachers', label: 'Đội ngũ chuyên môn', path: '/teachers' },
...
export default function Sidebar() {
  ...
  return (
    <aside className="sidebar fixed top-20 left-4 h-[calc(100vh-6rem)] rounded-3xl z-40 overflow-hidden overflow-x-hidden transition-all duration-300">
      {/* Nav Section */}
      <nav className="sidebar__nav flex-1 overflow-y-auto overflow-x-hidden py-2 custom-scrollbar pt-4">
        {MENU_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.path}
              prefetch={true}
              className={`sidebar__nav-item flex items-center px-4 py-3 rounded-2xl text-sm font-semibold transition-all duration-200 group gap-3 ${active ? 'sidebar__nav-item--active' : ''
                }`}
            >
              {item.label}
            </Link>
